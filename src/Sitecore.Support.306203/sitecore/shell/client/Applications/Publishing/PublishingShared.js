    ...
    function (Constants, Utils, handlebars, Preferences) {
        ...
        var enforceChecking = function (values, messageId) {
            ...
            if (values.length === 0) {
                ...
                var message = Utils.findItem(Constants.SitecoreSupportSearchDataSourceMessages.Items, messageId);
                // Sitecore.Support.306203 ---
        ...
        var toggleButtonPublish = function (messageBar, publishButtons) {
            ...
            var errorsDisplayed = messageBar.HasErrorMessages === undefined ? false : messageBar.HasErrorMessages;
            // Sitecore.Support.306203 ---



            // Sitecore.Support.306203 +++
            areAnyLangTargetSelected = false;

            var publishDalogWindow = this.PublishTabApp;

            if (!publishDalogWindow)
            {
                publishDalogWindow = this;
            }

            var shoulLangAndTargetBeAcounted = true;

            if (publishDalogWindow.CheckBoxListLanguages && publishDalogWindow.CheckBoxListTargets)
            {              
                var languages = publishDalogWindow.CheckBoxListLanguages.CheckedItems.map(function (t) {
                    return { "Code": t.Code, "DisplayName": t.DisplayName };
                    });
                var targets = publishDalogWindow.CheckBoxListTargets.CheckedItems.map(function (t) {
                    return { "Id": t.Code, "Name": t.DisplayName };
                    });

                    areAnyLangTargetSelected = languages.length > 0 && targets.length > 0;
            } 
            
            if (this.HeaderTitle)
            {
                if (this.HeaderTitle.Text === "Publishing Dashboard")
                {                
                shoulLangAndTargetBeAcounted = false;
                }
            }       
            // Sitecore.Support.306203 ---
            ...
            publishButtons.forEach(function(button) {
                if (button != undefined) {
                    ...
                    button.IsEnabled = (!errorsDisplayed && (areAnyLangTargetSelected || !shoulLangAndTargetBeAcounted));
                    // Sitecore.Support.306203 ---
                }
            });
        };
