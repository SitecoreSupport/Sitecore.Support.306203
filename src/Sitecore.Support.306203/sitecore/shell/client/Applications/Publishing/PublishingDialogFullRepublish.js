(function (Speak) {
    ...
    Speak.pageCode(["publishingUtils", "publishingConstants", "publishingShared"], function (Utils, Constants, Shared) {
        ...
        var toggleDisplay = function () {
            // Sitecore.Support.306203 +++
            //this.SearchDataSourceMessages.on("change:HasData", function () {
                //var message = Utils.findItem(this.SearchDataSourceMessages.Items, Constants.itemIds.fullRePublishWarning);
                var message = Utils.findItem(Constants.SitecoreSupportSearchDataSourceMessages.Items, Constants.itemIds.fullRePublishWarning);
        ...
        var toggleBorders = function (showPublish) {
            ...
            // Sitecore.Support.306203 +++
            this.MessageBar.IsVisible = !!showPublish;
            // Sitecore.Support.306203 ---
        }
        ...
        var executePublish = function () {
            ...
            newPublishJob[Constants.serviceParameters.languages] = this.CheckBoxListLanguages.CheckedItems.map(function (t) { return { "Code": t.Code, "DisplayName": t.DisplayName }; });
            newPublishJob[Constants.serviceParameters.targets] = this.CheckBoxListTargets.CheckedItems.map(function (t) { return { "Id": t.Code, "Name": t.DisplayName }; });
            newPublishJob[Constants.serviceParameters.syncWithTarget] = checkedItems.indexOf(syncWithTargetItem) !== -1;
            newPublishJob[Constants.serviceParameters.sourceDatabase] = Sitecore.Speak.Context.current().contentDatabase;

            newPublishJob[Constants.serviceParameters.metadata] = {};
            newPublishJob[Constants.serviceParameters.metadata][Constants.serviceParameters.clearAllCaches] = this.CheckBoxClearCache.IsChecked.toString();
            newPublishJob[Constants.serviceParameters.metadata][Constants.serviceParameters.fullRepublish] = this.CheckBoxFullRepublish.IsChecked.toString();
            ...
            $.ajax({
              ...
              .fail(function (jqXHR) {
                  ...
                  if (jqXHR.status === 401) {
                      ...
                      // Sitecore.Support.306203 +++
                      //var message = Utils.findItem(dialog.SearchDataSourceMessages.Items, Constants.itemIds.publishError);
                      var message = Utils.findItem(Constants.SitecoreSupportSearchDataSourceMessages.Items, Constants.itemIds.publishError);
                      // Sitecore.Support.306203 ---
